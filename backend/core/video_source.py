...
def parse_miuistore_field(page_html: str, label: str) -> str | None:
    pattern = re.compile(
        rf"{re.escape(label)}：\s*</div>\s*<div[^>]*class=['\"]col col-value['\"][^>]*>([\s\S]*?)</div>",
        re.I,
    )
...
def choose_yt_dlp_format(info: dict[str, Any]) -> str | None:
    ...
    return str(formats[0]["url"]) if formats else None


def resolve_with_yt_dlp(url: str) -> ResolvedVideo | None:
    try:
        info = run_yt_dlp(url)
        download_url = choose_yt_dlp_format(info)
        if not download_url:
            return None
        return ResolvedVideo(
            provider="yt-dlp",
...
def download_file(url: str, file_path: Path, on_progress: ProgressCallback | None = None) -> int:
    parse_http_url(url)
    request = urllib.request.Request(
        url,
        headers={
