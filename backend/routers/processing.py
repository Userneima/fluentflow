        ...
        try:
            async for chunk in event_stream():
                ...
                if H.JobEventHub.is_terminal(event):
                    terminal_sent = True
                await H.JOB_EVENTS.publish(task_id_value, event)
        except asyncio.CancelledError:
            terminal_sent = True
            ...
            if not terminal_sent:
                job = H.get_job(task_id_value)
                if job and job.get("status") in {"completed", "failed", "cancelled"}:
                    await H.JOB_EVENTS.publish(task_id_value, H.JobEventHub.event_from_job(job))

    await H.JOB_EVENTS.start(task_id_value, run_background_job)
