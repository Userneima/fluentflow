...
FLUENTFLOW_QUEUE_STEP_LEASE_SECONDS=900
FLUENTFLOW_QUEUE_HEARTBEAT_SECONDS=60
FLUENTFLOW_STALE_JOB_SECONDS=90000
```

后台队列的运行 step 会每 60 秒续租一次 15 分钟租约。服务进程重启时会立即恢复未完成
step；如果 worker 异常停止且没有重启，空闲 worker 也会在下一次轮询时回收过期租约。完成或
失败写入必须携带当前租约 owner，因此旧 worker 即使晚到也不能覆盖取消或新 worker 的结果。
