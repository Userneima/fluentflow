...
- 新增「视频链接下载登录态」设置（本机模式）：下载 B 站等链接时，可选择从 Chrome/Edge/Firefox/Safari/Brave 读取你的登录 cookie 来获取高清（1080p+ 需登录）。**默认关闭**；仅在本机读取、不上传，需先在该浏览器登录对应网站。底层沿用 yt-dlp `--cookies-from-browser`，原环境变量 `YT_DLP_COOKIES_FROM_BROWSER` 仍作为回退。B 站需登录的失败提示也改为指向该设置，不再暴露环境变量名。
