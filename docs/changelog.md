...
- 历史记录进一步收敛为后端任务记录的镜像：刷新后不再从旧浏览器 `fluentflow_history` 恢复结果，最近任务、编辑器和 Agent 工作流优先通过真实任务读取结果。
...
- 后台任务阶段更新会保留队列参数、来源路径和访客试用归属信息，避免访客任务开始处理后 `/guest-trial/jobs/{id}` 中途变成 404。
...
- Agent 工作流页会优先读取任务结果里的 `processing_plan`，展示真实的材料类型、执行范围、工具、计划步骤、计划依据和风险提示；旧任务没有计划时才使用前端兜底解释。
- Agent 工作流页从通用执行步骤列表进一步收敛为“当前素材特有的判断”：优先展示材料判断、路线选择、笔记策略、对结果的影响和复查点，通用时间线只作为辅助记录。
- 编辑器 AI 摘要底部的生成说明和“生成详情”入口下线，提示词模板、生成模式、分块数、摘要依据和原因说明不再挤占笔记正文空间；需要解释时通过“查看 Agent 工作流”进入任务解释页。
- 编辑器顶部“重新生成”统一改为“重生笔记”，并与“重新转录”一样先弹出二次确认，避免误触后覆盖当前笔记或转录。
- 编辑器中的“重生笔记”相关失败提示、后台任务建议和 Agent 工作流恢复动作统一为同一套动作文案，避免同一个操作在不同页面出现多个名字。
- 编辑器左侧新增“文本校对 / 视频复查”切换；有原视频和时间戳时可在视频下方直接编辑当前字幕，并用上一句 / 下一句快速跳转。
- 原视频默认短期保留 7 天，用于编辑器视频复查和“导出”菜单下载原视频；可通过 `FLUENTFLOW_SOURCE_RETENTION_DAYS=0` 恢复完成后立即删除。
- 笔记正文支持显示 Agent 选中的视频关键截图，并提供轻量“隐藏截图 / 显示截图”开关，避免截图长期挤压正文阅读空间。
- 视频关键截图抽取新增低变化兜底：当 scene 检测没有命中画面变化时，会按视频时长均匀抓取候选帧，避免短视频或低运动课程没有任何插图。
- 视频关键截图会经过全局密度、重复画面和低价值页面过滤；封面、目录、纯标题页、过渡帧或重复截图不会继续挤占最终笔记。
...
- Bilibili 链接解析不再使用 miuistore 第三方兜底；`yt-dlp` 解析失败时直接提示上传本地视频或配置浏览器 cookies 后重试。
...
- `/processing` 接入 `processing_plan.material`、`execution`、`note_strategy`、`steps`、`risk_notes` 和 `material.evidence`，并补充测试保护该数据优先级。
- 编辑器保留生成元数据给 Agent 工作流页读取，但默认阅读区不再渲染生成详情；新增测试保护 Agent 工作流入口、重生笔记确认弹窗、重新转录确认弹窗和旧文案不回流。
- `frontend/src/app/i18n.jsx` 改为从 `shared.jsx` 转导出 i18n，避免拆分后存在两份翻译表导致按钮文案和失败建议再次分叉。
- `docs/ui_design_system.md` 新增 Secondary Surface Consistency 规则，并补测试防止登录页、关于页、提示词弹窗和设置页维护区回退到旧 UI 语言。
...
- 前端和后端都移除本机历史候选扫描与账号导入旧路线；`/account/import-history` 和 `/local-history/candidates` 仅保留 410 Gone 墓碑响应，避免旧客户端误以为还能导入。
