...
| 概念 | 第一版规则 |
| --- | --- |
| `task_id` | 云端在任务登记时生成；桌面端将其作为本地任务 ID 使用，避免双 ID 映射和碰撞 |
...
桌面端在开始处理前创建云端任务，提交 `device_id`、用户可见源元数据、`execution_location` 和一个
随机 `idempotency_key`。云端以“账号 + device + idempotency key”去重，返回稳定 `task_id` 和
`result_revision = 0`。
...
| 1C. 桌面同步客户端 | 本地配置、任务提交、离线重试、状态 UI | 进行中：已完成本机凭据与浏览器配对基础；待接入任务提交、离线重试与状态 UI |
...
1A/1B 已完成，1C 已完成安全配对基础，但尚未开放桌面任务自动同步、离线重试或跨设备编辑。
