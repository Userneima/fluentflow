...
| 工作单元 | 主要表面 | 结束条件 |
| --- | --- | --- |
| 1A. 云端设备凭据 | account auth、设备存储、撤销 API、测试 | 设备只能代表自己的账号，撤销后立即失效 |
...
本文完成了阶段 0 的设计输出，但不代表可以跳过地基稳定化验收。阶段 1 的代码改动只能在上述本地和
云端真实链路人工验收记录完成后开始；届时每个工作单元单独验证、提交和审查。
