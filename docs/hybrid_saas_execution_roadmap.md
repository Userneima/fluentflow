...
状态：阶段 1 进行中，2026-07-20

阶段 0 状态：已完成。实现契约见 `docs/hybrid_execution_sync_contract.md`；阶段 1 已完成 1A（桌面
设备凭据）、1B（幂等任务/结果同步存储）和 1C 的浏览器安全配对、本地 outbox 同步与离线重试基础；
尚未提供同步状态 UI、跨设备阅读 UI 或保留期清理。
...
- **账号删除**：用户请求删除账号后，云端账号数据进入 7 天清除期；在清理任务真正执行前允许用户撤销
  删除，期满后彻底清除。删除云端数据不能删除用户设备上的源视频或本地运行数据。
- **原视频默认本地**：原视频不随结果同步；只有用户主动选择跨设备播放时才上传 OSS，且 OSS 原视频自
  上传完成起默认保留 7 天。
- **第一版范围**：只支持从桌面启动器进入的“本地 STT + 云端同步结果”。线上域名已有的云端转录
  保持现状，不纳入第一版改造。
- **云提供商**：采用阿里云中国区。阶段 0 和第一版可复用现有云端服务；具体地域、ECS / 容器、
  RDS、OSS、网络和备份拓扑属于阶段 6 的正式 SaaS 运行底座设计。

阶段 0 只需把上述 7 天计时、清理幂等和撤销窗口落实为可测试的实现契约，不再有产品层面的保留期
决策阻塞第一版。
...
| --- | --- | --- | --- |
| 桌面启动器 | 第一版本地 STT；后续可选本地抽音频后云端 STT | 不上传即可在本机浏览器播放 | 任务状态、字幕、笔记、编辑结果、轻量产物（默认保留 7 天） |
...
| 1. 本地执行结果同步 MVP | 桌面端以云端账号登录；只使用本地 STT；完成后同步任务状态、字幕、笔记与编辑结果；原视频标记为“仅处理设备可用” | 两台设备登录同一账号可查看同一任务与结果；另一设备不会把无原视频误报为失败 | 3-5 周 |
| 2. 云端入口收敛 | 保持已有线上云端转录链路；补齐上传、处理、播放和 7 天保留提示，统一桌面与网页任务状态表达 | 网页端不出现“选择本地转录”这类不可执行承诺；长任务有明确失败与恢复语义 | 2-3 周 |
...
| 5. 可选 OSS 跨设备播放 | 原视频采用签名直传、分片、断点续传、校验、访问控制和生命周期清理；播放器使用短时授权 URL | 用户主动上传后才跨设备播放；OSS 原视频上传完成后 7 天清理；未上传时始终保留本地可用说明 | 4-6 周 |
| 6. 正式 SaaS 运行底座 | 从单机 SQLite / 进程内队列演进到托管关系数据库、可靠任务队列、对象存储、备份恢复、监控告警、数据删除与配额体系 | 可横向扩展；长任务重启恢复可预期；完成故障、备份和删除演练 | 5-8 周 |

阶段 1 是最小可用的跨设备价值，阶段 3 和 5 均可选。正式 SaaS 上线前不能跳过阶段 0 的数据
契约和阶段 6 的可靠性工作。
...
6. **隐私与删除**：任务完成 7 天后的云端结果清理；OSS 原视频上传完成 7 天后的清理；用户发起账号删除
   后的 7 天撤销窗口和期满彻底清除；本地源文件如何保持不受影响。
...
- 用户数据导出、7 天云端结果和 OSS 原视频保留、账号删除后 7 天内可撤销且期满彻底清除、隐私说明和客服排障边界。
...
在当前基础稳定化验收完成后，第一版“桌面本地 STT + 云端同步结果”预计需要 **4-6 周**；后续加入
压缩音频云端 STT、网页唤起本地处理、OSS 跨设备播放和正式 SaaS 运行底座，顺序串行估计为
**17-31 周**。这是单名全职工程师、现有产品继续维护的保守区间；两名能独立处理客户端和云端基础设施
的工程师可以并行部分工作，但账号、任务契约和数据迁移仍须由一个 owner 收敛。

阶段 0 已具备产品决策输入；其工作是把这些决定转化为 API、状态机、清理任务和测试契约。
