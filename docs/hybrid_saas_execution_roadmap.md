...
| --- | --- | --- | --- |
| 桌面启动器 | 本地 STT；或本地抽音频后云端 STT | 不上传即可在本机浏览器播放 | 任务状态、字幕、笔记、编辑结果、轻量产物 |
...
| --- | --- | --- | --- |
| 0. 解冻与契约 | 完成当前地基计划人工验收；定义任务归属、执行位置、源文件可用性、同步幂等、冲突、取消、重试、删除和保留策略 | 有评审过的 API / 数据契约和验收样例；不改用户流程 | 3-5 个工作日，外加当前解冻验收 |
...
6. **隐私与删除**：用户删除云端任务时是否删除结果；本地源文件如何保持不受影响；OSS 文件何时过期或彻底删除。
...
- 账号会话、设备登记 / 撤销、登录风险控制、限流、配额账本和支付前的用量统计。
- 日志、指标、告警、备份、恢复演练、迁移与回滚方案。
- 用户数据导出、删除、隐私说明、保留策略和客服排障边界。
...
2. 原视频是否默认永不上传，只有用户主动开启跨设备播放时才进 OSS。
3. 云端部署、对象存储和数据驻留是否确定采用阿里云中国区。
4. 线上网页与桌面端的账号是否统一使用现有 FluentFlow 账号体系，还是计划接入第三方登录作为首选。
