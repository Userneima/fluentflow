...
FluentFlow 的产品页顶部是操作型页面标题，不是营销 hero。除开始处理页的核心上传入口、空状态的首次行动提示、或明确的对外介绍页外，不要在应用内页面顶部放大面积品牌标题、宣传语或解释性段落。
...
- Agent、AI、自动化相关页面尤其要避免“看起来智能”的大标题区。Agent 的价值应通过真实计划、判断依据、状态和恢复动作呈现。
...
- 后台任务优先使用进度、监控、队列状态语义；编辑器优先使用编辑、铅笔、文本修改语义。
