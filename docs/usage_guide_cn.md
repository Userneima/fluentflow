...
FluentFlow 是一套“云端/本地转录 + AI 生成结构化笔记 + 可选飞书导出”的软件。
当前公开产品优先使用 ElevenLabs Scribe 云端转录；本地 faster-whisper 保留为本机使用、开发和应急路径。生成摘要与导出会调用外部 API。
...
填写完会保存在本机后端的本地配置文件中，不再写入浏览器 `localStorage`。
...
> 说明：转录（STT）是 CPU 密集型过程，进度条会在该阶段逐步变化。音视频转录运行在后台任务和独立子进程中；关闭页面或连接中断不会自动取消任务，重新打开后可恢复进度。点击取消后，后端会取消后台任务并终止 STT 子进程，避免已经取消的任务继续占用 CPU。

处理过的音视频源文件会保存在本机系统应用数据目录的 `sources/` 下，用于后续从历史结果重新转录。该目录只在本机使用，不会上传到云端。
...
### 6.2 选择“笔记生成模式”（控制长字幕怎么处理）

在“工作台 Workbench”的“摘要 AI”区域可以选择：

- `自动选择`：默认推荐。约 2 万字符以内直接生成，超过后自动使用高保真模式。
- `直接长上下文`：整段转录一次发送给模型，速度更快，适合较短材料。
- `高保真笔记`：先分段提取证据，再生成终稿并尽量检查遗漏，适合长课程，但会更慢。

这个阈值不是模型上下文上限，而是质量策略。后续可以用同一份字幕分别测试“直接长上下文”和“高保真笔记”，观察是否减少漏点。

### 6.3 导出到飞书（Lark/Feishu）
